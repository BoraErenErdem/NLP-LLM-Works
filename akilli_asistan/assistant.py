...
def detect_intent(user_message): # kullanıcıdan gelen mesajı niyet sınıflandırması ile sınıflandırıp ne istediğini anlamak..!
    prompt = f"""
        Kullanıcının aşağıdaki cümlesini sınıflandır:
        Etiketlerden sadece birini döndür:
        - not_ozet (eğer notlarla ilgili bir soru sorduysa yada not özetletmek istiyorsa)
        - etkinlik_ozet (eğer etkinliklerle ilgili bir soru sorduysa yada etkinlik özetletmek istiyorsa)
        - ozelgun_ozet (eğer özel günlerle ilgili bir soru yada özel günleri özetlemek istiyorsa)
        - normal (diğer her şey)
        
        Cümle: {user_message}
        Sadece etiket döndür: (örnek: not_ozet)
        """
